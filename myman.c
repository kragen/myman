...
#define YSTART (47 * TILEHEIGHT / 2)

#define XFRUIT XSTART
#define YFRUIT (35 * TILEHEIGHT / 2)

#define YGHOST (YFRUIT - 3 * TILEHEIGHT)

#define YTOP (YFRUIT - 5 * TILEHEIGHT)
...
int main(int argc, char **argv) {
  ...
  memcpy(maze[YMSG] + XMSG, READY, 10);
	 ...
	 ((k = getch()) != 'q') && (k != 'Q'))
  {
	...
	dead = YSTART - YGHOST;
	memcpy(maze[YMSG] + XMSG, lives ? READY : GAMEOVER, 10);
      }
      if (dying)
	sprite_frame[MYMAN] = ((DEATHDELAY - dying) >> DEATHSHIFT) % 4;
    } else if (dead) {
      if (! -- dead) {
	memcpy(maze[YMSG] + XMSG, BLANK, 10);
