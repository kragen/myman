...
#define MAZEFILE "catchum.txt"
#define WIDTH 25
#define HEIGHT 23
#define DOTS 203
#define DOTS_FRUIT1 45
#define DOTS_FRUIT2 90
#define MSGLEN 9
...
#define YSTART (35 * TILEHEIGHT / 2)
#define YFRUIT (27 * TILEHEIGHT / 2)
#define XFRUIT XSTART
#define YGHOST (YFRUIT - 2 * TILEHEIGHT)
#define YTOP (YFRUIT - 3 * TILEHEIGHT)
#define PAUSE " - PAUSED - "
	...
	if (dying || winning) {
	  ...
	  if (sprite_used[blue]) {
	    ...
	    o1 = ISOPEN(maze[YWRAP(j1 + YDIR(dir1))]
			[XWRAP(i1 + XDIR(dir1))]);
	    o2 = ISOPEN(maze[YWRAP(j1 + YDIR(dir2))][XWRAP(i1 + XDIR(dir2))]);
	    doory = ISDOOR(maze[YWRAP(j1 - 1)][i1]);
	    doorx = ISDOOR(maze[j1][XWRAP(i1 + 1)]);
	    d0 = ((dir0 == UP) && doory)
	      || ((dir0 == RIGHT) && doorx);
	    d1 = ((dir1 == UP) && doory)
	      || ((dir1 == RIGHT) && doorx);
